...
Docker Desktop for Linux uses a per-user socket instead of the system-wide `/var/run/docker.sock`. This means that Docker SDKs and tools that expect the default socket location need to be configured with the `DOCKER_HOST` environment variable to connect to Docker Desktop.

The `docker` CLI automatically uses the correct socket through the `desktop-linux` context, but SDKs require explicit configuration.

To set `DOCKER_HOST` for your current shell session:

```bash
export DOCKER_HOST=unix://$HOME/.docker/desktop/docker.sock
```

Alternatively, you can dynamically retrieve the socket path from the `desktop-linux` context:
...
After setting `DOCKER_HOST`, Docker SDKs and tools can connect to Docker Desktop for Linux.

For more information, see [How do I use Docker SDKs with Docker Desktop for Linux?](/manuals/desktop/troubleshoot-and-support/faqs/linuxfaqs.md#how-do-i-use-docker-sdks-with-docker-desktop-for-linux)
